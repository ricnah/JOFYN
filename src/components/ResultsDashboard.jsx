...
export default function ResultsDashboard({
  ...
  return (
    <div className="p-4 md:p-6 w-full h-full flex flex-col overflow-hidden animate-fadeIn">
      <div className="mb-4 glass-panel py-4 px-6 rounded-2xl border-white/60 shrink-0">
        <h2 className="text-2xl md:text-3xl font-extrabold text-violet-950 mb-1 tracking-tight">{currentTitle}</h2>
        <p className="text-violet-700 font-medium text-sm">{t('displaying_results')}</p>
      </div>

      {/* Stat Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 shrink-0">
        <div className="glass-panel border-white/60 p-4 rounded-xl shadow-sm">
          <p className="text-violet-600 text-xs font-extrabold uppercase tracking-wider">{t('total_jobs')}</p>
          <p className="text-3xl md:text-4xl font-black text-violet-800 mt-1">
            {currentData.length === 0 ? '. . .' : currentData.length}
          </p>
        </div>
        <div className="glass-panel border-white/60 p-4 rounded-xl shadow-sm">
          <p className="text-violet-600 text-xs font-extrabold uppercase tracking-wider">{t('top_platform')}</p>
          <p className="text-2xl font-black text-violet-800 mt-1">
            {currentData.length === 0 ? '. . .' : 'Glints'}
          </p>
        </div>
        <div className="glass-panel border-white/60 p-4 rounded-xl shadow-sm">
          ...
          <div className="flex items-center mt-1">
            <IconLocation />
            <p className="text-2xl font-black text-violet-800 ml-1">
              {currentData.length === 0 ? '. . .' : getMostFrequentLocation()}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Data Table */}
      <div className="glass-panel border-white/60 rounded-3xl overflow-hidden shadow-xl flex-grow flex flex-col min-h-0">
        <div className="overflow-auto flex-grow">
          <table className="w-full text-left text-sm text-violet-900 border-collapse">
            <thead className="sticky top-0 bg-white/95 backdrop-blur-md text-violet-700 uppercase text-[10px] font-black tracking-widest border-b border-white/50 z-20">
              <tr>
                <th className="px-5 py-4">{t('company')}</th>
                <th className="px-5 py-4">{t('role')}</th>
                <th className="px-5 py-4">{t('salary')}</th>
                ...
                <th className="px-5 py-4 text-center">{t('scraping_date')}</th>
            ...
            <tbody className="divide-y divide-white/40">
              {currentData.length === 0 ? (
                <tr className="hover:bg-white/10 transition-colors">
                  <td className="px-5 py-4 text-violet-400 font-bold font-mono">. . .</td>
                  <td className="px-5 py-4 text-violet-400 font-bold font-mono">. . .</td>
                  <td className="px-5 py-4 text-violet-400 font-bold font-mono">. . .</td>
                  <td className="px-5 py-4 text-violet-400 font-bold font-mono">. . .</td>
                  <td className="px-5 py-4">
                    <span className="px-2 py-1 text-[10px] uppercase bg-white/30 text-violet-400 rounded-md border border-white/30 font-mono">
                      . . .
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <span className="px-2.5 py-1 text-[11px] rounded-md border bg-white/30 text-violet-400 border-white/30 font-mono">
                      . . .
                    </span>
                  </td>
                  <td className="px-5 py-4 text-center">
                    <span className="px-3 py-2 text-[11px] rounded-xl bg-white/30 border border-white/30 text-violet-400 inline-block w-[135px] font-mono">
                      . . .
                    </span>
                  </td>
                  <td className="px-5 py-4 text-center text-[11px] font-mono text-violet-400">
                    . . .
                  </td>
                  <td className="px-5 py-4 text-center">
                    <span className="px-3.5 py-1.5 bg-white/30 border border-white/30 text-violet-400 rounded-xl text-xs inline-block font-mono">
                      . . .
                    </span>
                  </td>
                </tr>
                ...
                currentData.map(job => (
                  <tr key={job.id} className="hover:bg-white/30 transition-colors">
                    ...
                    <td className="px-5 py-4 text-center">
                      <button className="px-3.5 py-1.5 bg-white/60 border border-white/50 text-violet-900 rounded-xl hover:bg-violet-600 hover:text-white hover:border-violet-500 transition-all text-xs whitespace-nowrap shadow-sm">
                        {t('open_link')}
                      </button>
                    </td>
                  </tr>
